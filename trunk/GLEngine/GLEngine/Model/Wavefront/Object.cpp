...
Object::Object(std::istream& modelStream, unsigned int previewMaxFacesIndex)
		throw (gl::GLException) {
	if (!modelStream.good() || modelStream.eof()) {
		throw gl::GLException(
				"ObjModel::ObjModel - Wrong argument data is empty");
	}
	...
	do { // Start reading file data
		...
		if (tokens[0] == CONST_VERTECES_TOKEN) { // The first character is a v: on this line is a vertex stored.
			if (tokens.size() != 5) {
				throw gl::GLException(
						"ObjModel::ObjModel - Wrong stream format");
			}

			try {
				_vertexBuffer.push_back(
						utils::StrUtil::lexical_cast<float>(tokens[1]));
				_vertexBuffer.push_back(
						utils::StrUtil::lexical_cast<float>(tokens[2]));
				_vertexBuffer.push_back(
						utils::StrUtil::lexical_cast<float>(tokens[3]));
			...
			try {
				vertexNumber[0] = utils::StrUtil::lexical_cast<unsigned int>(
						tokens[1]);
				vertexNumber[1] = utils::StrUtil::lexical_cast<unsigned int>(
						tokens[2]);
				vertexNumber[2] = utils::StrUtil::lexical_cast<unsigned int>(
						tokens[3]);
			...
			for (unsigned int i = 0; i < CONST_POINTS_PER_VERTEX; i++) {
				...
				_triangleFaces.push_back(
						_vertexBuffer[3 * vertexNumber[i] + 1]);
				_triangleFaces.push_back(
						_vertexBuffer[3 * vertexNumber[i] + 2]);
			 ...
			 */
			float coord1[3] = { _triangleFaces[triangleIndex],
					_triangleFaces[triangleIndex + 1],
					_triangleFaces[triangleIndex + 2] };
			float coord2[3] = { _triangleFaces[triangleIndex + 3],
					_triangleFaces[triangleIndex + 4],
					_triangleFaces[triangleIndex + 5] };
			float coord3[3] = { _triangleFaces[triangleIndex + 6],
					_triangleFaces[triangleIndex + 7],
					_triangleFaces[triangleIndex + 8] };
	...
	} while (!modelStream.eof() && tokens[0] != CONST_OBJECT_TOKEN
			&& tokens[0] != CONST_GROUP_TOKEN);
...
bool Object::drawObject() {
	...
	try {
		...
		for (currentRotation = _rotations.begin();
				currentRotation != _rotations.end(); currentRotation++) {
			glRotatef(currentRotation->second.getAngle(),
					currentRotation->second.getAxisX(),
					currentRotation->second.getAxisY(),
					currentRotation->second.getAxisZ());
		}
	...
	glDrawArrays(GL_TRIANGLES, 0,
			_triangleFaces.size() / CONST_POINTS_PER_VERTEX); // Draw the triangles
...
unsigned int Object::getLastVertexNumber() {
	...
	try {
		utils::AutoLock lock(_lockObject);
		std::vector<unsigned int>::iterator res = std::max_element(
				_vertexNumbers.begin(), _vertexNumbers.end());
