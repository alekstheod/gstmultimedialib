...
void utils::XmlNode::readAttributes(std::wstring& stream) throw (XmlException) {
	StrUtil::eatTrim(stream, CONST_XML_DELIMITERS);
	while (!stream.empty() && !isEndOfTag(stream)
			&& stream[0] != CONST_TAG_END_SYMBOL) {
		std::wstring attributeName = readAttributeName(stream);
		std::wstring attributeValue = readAttributeValue(stream);
		std::pair<std::wstring, std::wstring> newEntry(attributeName,
				attributeValue);
...
bool utils::XmlNode::isEndOfTag(std::wstring& stream) throw (XmlException) {
	StrUtil::eatTrim(stream, CONST_XML_DELIMITERS);
	if (stream.length() > 2 && stream[0] == CONST_TAG_CLOSE_SYMBOL
			&& stream[1] == CONST_TAG_END_SYMBOL) {
...
std::wstring utils::XmlNode::readAttributeName(std::wstring& stream)
		throw (XmlException) {
	StrUtil::eatTrim(stream, CONST_XML_DELIMITERS);
	std::wstring argName;
	unsigned int pos = stream.find_first_of(L" \r\n><'=");
...
std::wstring utils::XmlNode::readAttributeValue(std::wstring& stream)
		throw (XmlException) {
	StrUtil::eatTrim(stream, CONST_XML_DELIMITERS);
	if (stream.empty()) {
		throw XmlException("Wrong XML string");
	...
	if (CONST_TAG_ARGUMENT_VALUE_DELIMITERS.find(stream[0])
			!= CONST_TAG_ARGUMENT_VALUE_DELIMITERS.npos) {
...
bool utils::XmlNode::isACloseTagSymbols(std::wstring& stream) {
	if (stream.length() < 2 || stream[0] != CONST_TAG_BEGIN_SYMBOL
			|| stream[1] != CONST_TAG_CLOSE_SYMBOL) {
		return false;
	}
	return true;
}
