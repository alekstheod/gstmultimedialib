...
template<class T>
Bitmap<T>::Bitmap(unsigned int width, unsigned int height):_bmpInfo(0) {
	T newPixel;
...
bool Bitmap<T>::setHeight(unsigned int height) {
	...
	if (_bmpInfo.biHeight <= height) {
		...
		for (unsigned int i = height - _bmpInfo.biHeight; i < _bmpInfo.biHeight;
				i++) {
...
std::vector<unsigned char> Bitmap<T>::getBitmapAsRawData(void) const {
	...
	unsigned int pad = (4
			- _bmpInfo.biWidth * (tmpPixel->getPixelDataSize()) % 4);
...
std::vector<unsigned char> Bitmap<T>::getBitmapAsFileRawData(void) const {
	...
	std::copy( &_bmpInfo, &_bmpInfo+( sizeof(_bmpInfo) ), result.begin() );
	T p;
	IPixel* tmpPixel = static_cast<IPixel*>(&p);
	unsigned int pad = (4
			- _bmpInfo.biWidth * (tmpPixel->getPixelDataSize()) % 4);
