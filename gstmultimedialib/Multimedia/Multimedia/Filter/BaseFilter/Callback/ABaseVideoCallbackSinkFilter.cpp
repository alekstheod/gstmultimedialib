...
ABaseVideoCallbackSinkFilter::ABaseVideoCallbackSinkFilter(
	...
	g_object_set(G_OBJECT(_output.getPtr()), render_video_callback_property,
			renderCallback, NULL);
	g_object_set(G_OBJECT(_output.getPtr()), setcaps_video_callback_property,
			setCapsCallback, NULL);
	g_object_set(G_OBJECT(_output.getPtr()), chain_video_callback_arg_property,
			this, NULL);
}
