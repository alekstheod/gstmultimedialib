...
bool Model::drawModel() {
	...
	for (curRotation = _rotations.begin(); curRotation != _rotations.end();
			curRotation++) {
		glRotatef(curRotation->second.getAngle(),
				curRotation->second.getAxisX(), curRotation->second.getAxisY(),
				curRotation->second.getAxisZ());
	}

	std::map<std::string, Object>::iterator curObject;
	for (curObject = _glObjects.begin(); curObject != _glObjects.end();
			curObject++) {
...
bool Model::addRotationX(const std::string& objectName, unsigned int rotationId,
		float angle) {
	bool result = true;
	try {
		utils::AutoLock lock(_lockObject);
		std::map<std::string, Object>::iterator currentObject = _glObjects.find(
				objectName);
...
bool Model::addRotationY(const std::string& objectName, unsigned int rotationId,
		float angle) {
	bool result = true;
	try {
		utils::AutoLock lock(_lockObject);
...
bool Model::addRotationZ(const std::string& objectName, unsigned int rotationId,
		float angle) {
	bool result = true;
	try {
		utils::AutoLock lock(_lockObject);
