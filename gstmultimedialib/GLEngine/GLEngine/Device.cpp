...
bool Device::generateTexture ( utils::SharedPtr<Texture>& newTexture )
{
    ...
    if ( _textures.empty() ) {
        ...
        _textures.insert ( std::make_pair(texture, newTexture) );
    } else {
        std::map<GLuint, utils::SharedPtr<Texture> >::iterator prevTexture;
        for ( auto curTexture = _textures.begin();
        ...
        if ( texture != CONST_INVALID_TEXTURE ) {
            newTexture.Attach ( new Texture ( texture ) );
            result = _textures.insert ( std::make_pair(texture, newTexture) ).second;
        } else {
            auto curTexture = ( --_textures.end() );
